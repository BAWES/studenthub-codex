// ---------------------------------------------------------------------------
// E2E Smoke: Candidate payments pages
//
// Tests /candidate/payments list page and /candidate/payments/[id] detail
...
test.describe("Candidate payments pages", () => {
  ...
  test("list page loads without errors", async () => {
    ...
    await ctx.page.goto("/candidate/payments");
  ...
  test("admin is redirected away from payments list", async () => {
    const ctx = await authContext(admin);
    await ctx.page.goto("/candidate/payments");
    await ctx.page.waitForLoadState("load");
    expect(ctx.page.url()).not.toContain("/candidate/payments");
    await ctx.close();
  });

  test("company is redirected away from payments list", async () => {
