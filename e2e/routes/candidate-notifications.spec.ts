// ---------------------------------------------------------------------------
// E2E Smoke: Candidate notifications pages
//
// Tests /candidate/notifications list page and /candidate/notifications/[id]
// detail page. CI only. Uses USE_MOCK_FIXTURES=true.
// ---------------------------------------------------------------------------
...
test.describe("Candidate notifications pages", () => {
  ...
  test("list page loads without errors", async () => {
    ...
    await ctx.page.goto("/candidate/notifications");
  ...
  test("admin is redirected away from notifications list", async () => {
    const ctx = await authContext(admin);
    await ctx.page.goto("/candidate/notifications");
    await ctx.page.waitForLoadState("load");
    expect(ctx.page.url()).not.toContain("/candidate/notifications");
    await ctx.close();
  });

  test("company is redirected away from notifications list", async () => {
