// ---------------------------------------------------------------------------
// E2E Smoke: Candidate schedule pages
//
// Tests /candidate/schedule list page and /candidate/schedule/[id] detail
...
test.describe("Candidate schedule pages", () => {
  ...
  test("list page loads without errors", async () => {
    ...
    await ctx.page.goto("/candidate/schedule");
  ...
  test("admin is redirected away from schedule list", async () => {
    const ctx = await authContext(admin);
    await ctx.page.goto("/candidate/schedule");
    await ctx.page.waitForLoadState("load");
    expect(ctx.page.url()).not.toContain("/candidate/schedule");
    await ctx.close();
  });

  test("company is redirected away from schedule list", async () => {
