// ---------------------------------------------------------------------------
// E2E Smoke: Candidate skills pages
//
// Tests /candidate/skills list page and /candidate/skills/new page. CI only.
// Uses USE_MOCK_FIXTURES=true.
// ---------------------------------------------------------------------------
...
test.describe("Candidate skills pages", () => {
  ...
  // ── New page (skills has a dedicated /new sub-route) ──

  test("new page loads without errors", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/skills/new");
    await ctx.page.waitForLoadState("load");
    await expect(ctx.page.locator("body")).toBeVisible({ timeout: 15000 });
    assertNoReactErrors(ctx.errors);
    await ctx.close();
  ...
  test("new page renders heading or content", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/skills/new");
    await ctx.page.waitForLoadState("load");
    const hasContent = await ctx.page.locator("h1, h2, h3, main, [class*='container'], [class*='form'], form").first().isVisible().catch(() => false);
    expect(hasContent).toBe(true);
    assertNoReactErrors(ctx.errors);
    await ctx.close();
  });
  ...
  test("admin is redirected away from skills list", async () => {
    const ctx = await authContext(admin);
    await ctx.page.goto("/candidate/skills");
    await ctx.page.waitForLoadState("load");
    expect(ctx.page.url()).not.toContain("/candidate/skills");
    await ctx.close();
  });

  test("company is redirected away from skills list", async () => {
