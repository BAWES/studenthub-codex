// ---------------------------------------------------------------------------
// E2E Sprint: 3-click navigation audit — Company role
//
// CI only. Uses USE_MOCK_FIXTURES=true to bypass DB dependency.
// Verifies all company workspace features (companies, contacts, requests,
// stores, workspace, employer) are reachable within ≤3 clicks from the hub.
// ---------------------------------------------------------------------------
...
test.describe("3-click audit — Company", () => {
  ...
  // Cross-role access check — other roles should not access company pages
  test("cross-role guard — other roles cannot access company pages", async () => {
    const fixtures = getMockFixtures();
