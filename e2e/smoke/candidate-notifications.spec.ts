// ---------------------------------------------------------------------------
// E2E Smoke: Candidate notifications page
//
// CI only. Uses USE_MOCK_FIXTURES=true to bypass DB dependency.
// Verifies the notifications page renders with expected UI elements.
// ---------------------------------------------------------------------------
...
test.describe("Candidate notifications page", () => {
  ...
  test("notifications page loads without errors", async () => {
  ...
  test("notifications page renders heading or list container", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/notifications");
    await ctx.page.waitForLoadState("load");

    // Should render either a heading or notification list
    const hasHeading = await ctx.page.locator("h1, h2, h3").first().isVisible().catch(() => false);
    const hasList = await ctx.page.locator("ul, [role='list'], table").first().isVisible().catch(() => false);
