...
test.describe("Admin payments page", () => {
  ...
  test("admin payments page loads without errors", async () => {
  ...
  test("admin payments page renders heading or table", async () => {
    const ctx = await authContext(admin);
    await ctx.page.goto("/admin/payments");
    await ctx.page.waitForLoadState("load");
    const hasContent = await ctx.page.locator("h1, h2, table, main, [role='list']").first().isVisible().catch(() => false);
