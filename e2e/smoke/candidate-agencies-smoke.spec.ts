// ---------------------------------------------------------------------------
// E2E Smoke: Candidate agencies page
//
// CI only. Uses USE_MOCK_FIXTURES=true to bypass DB dependency.
// Verifies /candidate/agencies loads without React
...
test.describe("Candidate agencies page", () => {
  ...
  test("candidate agencies page loads without errors", async () => {
  ...
  test("candidate agencies page renders heading or content", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/agencies");
    await ctx.page.waitForLoadState("load");
  ...
  test("candidate agencies page shows Agencies heading", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/agencies");
    await ctx.page.waitForLoadState("load");
    const heading = ctx.page.locator("h1, h2").filter({ hasText: "Agencies" });
    await expect(heading.first()).toBeVisible({ timeout: 10000 });
    assertNoReactErrors(ctx.errors);
    await ctx.close();
  });

  test("staff is redirected from candidate agencies (cross-role guard)", async () => {
    ...
    await page.goto("/candidate/agencies");
    await page.waitForLoadState("load");
    await expect(page).not.toHaveURL("/candidate/agencies");
