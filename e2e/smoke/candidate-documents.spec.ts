// ---------------------------------------------------------------------------
// E2E Smoke: Candidate documents page
//
...
test.describe("Candidate documents page", () => {
  ...
  test("documents page loads without errors", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/documents");
  ...
  test("documents page renders heading or list", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/documents");
    await ctx.page.waitForLoadState("load");
