// ---------------------------------------------------------------------------
// E2E Smoke: Candidate invitations page
//
...
test.describe("Candidate invitations page", () => {
  ...
  test("invitations page loads without errors", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/invitations");
  ...
  test("invitations page renders list or heading", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/invitations");
    await ctx.page.waitForLoadState("load");
    const hasContent = await ctx.page.locator("h1, h2, [role='list'], table, main").first().isVisible().catch(() => false);
