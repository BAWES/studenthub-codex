...
test.describe("Candidate edit page", () => {
  ...
  test("edit page loads without errors", async () => {
  ...
  test("edit page renders content container", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/edit");
    await ctx.page.waitForLoadState("load");
    const hasContent = await ctx.page.locator("h1, h2, [role='list'], table, main, form").first().isVisible().catch(() => false);
