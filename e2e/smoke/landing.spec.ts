...
test.describe("Landing page smoke tests (STU-N landing page redesign)", () => {
  ...
  test("hero feature pills render with staff-matched references", async ({ page }) => {
    ...
    await expect(page.locator("text=For students").first()).toBeVisible();
    await expect(page.locator("text=Profile visible to employers").first()).toBeVisible();
    await expect(page.locator("text=Staff-matched role suggestions").first()).toBeVisible();
    // Employer benefits section
    await expect(page.locator("text=For employers").first()).toBeVisible();
    await expect(page.locator("text=Staff-matched candidate suggestions").first()).toBeVisible();
  });
