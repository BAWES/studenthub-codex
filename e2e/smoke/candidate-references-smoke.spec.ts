// ---------------------------------------------------------------------------
// E2E Smoke: Candidate references page
//
// CI only. Uses USE_MOCK_FIXTURES=true to bypass DB dependency.
// Verifies /candidate/references loads without React
...
test.describe("Candidate references page", () => {
  ...
  test("candidate references page loads without errors", async () => {
  ...
  test("candidate references page renders heading or content", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/references");
    await ctx.page.waitForLoadState("load");
  ...
  test("candidate references page shows References heading", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/references");
    await ctx.page.waitForLoadState("load");
  ...
  test("staff is redirected from candidate references (cross-role guard)", async () => {
    ...
    await page.goto("/candidate/references");
    await page.waitForLoadState("load");
    await expect(page).not.toHaveURL("/candidate/references");
