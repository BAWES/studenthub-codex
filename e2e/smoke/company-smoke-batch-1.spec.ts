// ---------------------------------------------------------------------------
// E2E Smoke Batch 1: Company companies, company-settings, contacts
//
...
test.describe("Company smoke batch 1 — companies, company-settings, contacts", () => {
  ...
  test("candidate is redirected from company companies (cross-role guard)", async () => {
    ...
    await expect(page).not.toHaveURL("/company/companies");
    await bContext.close();
    await browser.close();
  });

  test("candidate is redirected from company settings (cross-role guard)", async () => {
    const browser = await (await import("@playwright/test")).chromium.launch();
    const bContext = await browser.newContext();
    await bContext.addCookies([
      { name: "studenthub_next_session", value: candidate.cookie, domain: "127.0.0.1", path: "/" },
    ...
    const page = await bContext.newPage();
    await page.goto("/company/company-settings");
    await page.waitForLoadState("load");
    await expect(page).not.toHaveURL("/company/company-settings");
    await bContext.close();
    await browser.close();
  });

  test("candidate is redirected from company contacts (cross-role guard)", async () => {
    const browser = await (await import("@playwright/test")).chromium.launch();
    const bContext = await browser.newContext();
    await bContext.addCookies([
      { name: "studenthub_next_session", value: candidate.cookie, domain: "127.0.0.1", path: "/" },
    ...
    await page.waitForLoadState("load");
    await expect(page).not.toHaveURL("/company/contacts");
    await bContext.close();
    await browser.close();
  });
});
