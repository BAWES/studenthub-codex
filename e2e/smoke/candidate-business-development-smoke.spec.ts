// ---------------------------------------------------------------------------
// E2E Smoke: Candidate business-development page
//
// CI only. Uses USE_MOCK_FIXTURES=true to bypass DB dependency.
// Verifies /candidate/business-development loads without React
...
test.describe("Candidate business-development page", () => {
  ...
  test("candidate business-development page loads without errors", async () => {
  ...
  test("candidate business-development page renders heading or content", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/business-development");
    await ctx.page.waitForLoadState("load");
  ...
  test("candidate business-development page shows Business Development heading", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/business-development");
    await ctx.page.waitForLoadState("load");
    const heading = ctx.page.locator("h1, h2").filter({ hasText: "Business Development" });
    await expect(heading.first()).toBeVisible({ timeout: 10000 });
    assertNoReactErrors(ctx.errors);
    await ctx.close();
  });

  test("staff is redirected from candidate business-development (cross-role guard)", async () => {
    ...
    await page.goto("/candidate/business-development");
    await page.waitForLoadState("load");
    await expect(page).not.toHaveURL("/candidate/business-development");
