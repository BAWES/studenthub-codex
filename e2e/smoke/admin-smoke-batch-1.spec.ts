...
test.describe("Admin smoke — departments, stores, tags, tickets", () => {
  ...
  for (const { path, name } of pages) {
    ...
    test(`${name} page renders content`, async ({ browser }) => {
      ...
      const page = await context.newPage();
      await page.goto(path);
      // The shell should render nav, heading, or table content
...
test.describe("Admin smoke — attendance, bank, reports", () => {
  test.describe.configure({ mode: "serial" });
...
test.describe("Admin smoke — employees, aws", () => {
  test.describe.configure({ mode: "serial" });
