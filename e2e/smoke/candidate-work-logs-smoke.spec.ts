// ---------------------------------------------------------------------------
// E2E Smoke: Candidate work-logs page
//
// CI only. Uses USE_MOCK_FIXTURES=true to bypass DB dependency.
// Verifies /candidate/work-logs loads without React
...
test.describe("Candidate work-logs page", () => {
  ...
  test("candidate work-logs page loads without errors", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/work-logs");
  ...
  test("candidate work-logs page renders heading or content", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/work-logs");
    await ctx.page.waitForLoadState("load");
  ...
  test("candidate work-logs page shows Work Logs heading", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/work-logs");
    await ctx.page.waitForLoadState("load");
    const heading = ctx.page.locator("h1, h2").filter({ hasText: /Work.?Log/i });
    await expect(heading.first()).toBeVisible({ timeout: 10000 });
    assertNoReactErrors(ctx.errors);
    await ctx.close();
  });

  test("staff is redirected from candidate work-logs (cross-role guard)", async () => {
    ...
    await expect(page).not.toHaveURL("/candidate/work-logs");
