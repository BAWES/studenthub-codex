// ---------------------------------------------------------------------------
// E2E Smoke: Candidate schedule page
//
...
test.describe("Candidate schedule page", () => {
  ...
  test("schedule page loads without errors", async () => {
  ...
  test("schedule page renders heading or calendar container", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/schedule");
    await ctx.page.waitForLoadState("load");
    const hasContent = await ctx.page.locator("h1, h2, main, table, [role='grid']").first().isVisible().catch(() => false);
