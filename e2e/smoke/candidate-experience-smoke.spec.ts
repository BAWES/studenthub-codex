// ---------------------------------------------------------------------------
// E2E Smoke: Candidate experience page
//
// CI only. Uses USE_MOCK_FIXTURES=true to bypass DB dependency.
// Verifies /candidate/experience loads without React
...
test.describe("Candidate experience page", () => {
  ...
  test("candidate experience page loads without errors", async () => {
  ...
  test("candidate experience page renders heading or content", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/experience");
    await ctx.page.waitForLoadState("load");
  ...
  test("candidate experience page shows Experience heading", async () => {
    const ctx = await authContext(candidate);
    await ctx.page.goto("/candidate/experience");
    await ctx.page.waitForLoadState("load");
    const heading = ctx.page.locator("h1, h2").filter({ hasText: "Experience" });
    await expect(heading.first()).toBeVisible({ timeout: 10000 });
    assertNoReactErrors(ctx.errors);
    await ctx.close();
  });

  test("staff is redirected from candidate experience (cross-role guard)", async () => {
    ...
    await expect(page).not.toHaveURL("/candidate/experience");
