...
test.describe("Company notes page", () => {
  ...
  test("company notes page loads without errors", async () => {
  ...
  test("company notes page renders heading or content", async () => {
    const ctx = await authContext(company);
    await ctx.page.goto("/company/notes");
    await ctx.page.waitForLoadState("load");
    const hasContent = await ctx.page.locator("h1, h2, table, [role='list'], main, ul, ol").first().isVisible().catch(() => false);
