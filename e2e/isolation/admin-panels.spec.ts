// ---------------------------------------------------------------------------
// E2E Isolation: Admin workspace panels — load independently
//
// Tests that each admin panel (Dashboard, Candidates, Companies, Requests,
// Payments, Transfers) loads independently with its own heading, and no
// content from other panels leaks into the viewport.
//
...
test.describe("Admin workspace panel isolation", () => {
  ...
  for (const panel of panels) {
    test(`${panel.label} panel loads independently`, async () => {
      ...
      const heading = ctx.page.locator(`h1, h2, h3`).filter({ hasText: panel.heading }).first();
      ...
      for (const other of panels) {
        ...
        const otherHeading = ctx.page.locator(`h1, h2, h3`).filter({ hasText: other.heading });
