...
async function expectBodyIncludes(
  path,
  expected,
  expectedText,
  cookie,
  ...
  if (!text.includes(expectedText)) {
    throw new Error(
      `${path} expected body to include ${JSON.stringify(expectedText)}`,
    );
  }
  const budgetMs = options.budgetMs ?? defaultBudgetMs;
  if (durationMs > budgetMs) {
...
async function expectOneOf(path, expectedStatuses, cookie, options = {}) {
  ...
  if (!expectedStatuses.includes(status)) {
    throw new Error(
      `${path} expected one of ${expectedStatuses.join(", ")}, got ${status}`,
    );
  }
  const budgetMs = options.budgetMs ?? defaultBudgetMs;
  if (durationMs > budgetMs) {
...
async function main() {
  ...
  const candidateInvitationCookie = signSession({
    ...
    email:
      candidateInvitation.candidate?.candidate_email ??
      "candidate-smoke@studenthub.local",
  });
  const candidateWorkLogCookie = signSession({
    ...
    email:
      candidateWorkLog.candidate?.candidate_email ??
      "candidate-smoke@studenthub.local",
  });
  const companyCookie = signSession({
    ...
    email:
      contactCompany.contact?.contact_email ?? "company-smoke@studenthub.local",
  });
  const companyRequestCookie = signSession({
    ...
    email:
      contactRequest.contact?.contact_email ?? "company-smoke@studenthub.local",
  });
  ...
  await expectStatus(
    `/app?scope=people&record=candidate-${adminCandidate.candidate_id}`,
    200,
    adminCookie,
  );
  await expectStatus("/hub", 200, adminCookie);
  await expectStatus(
    `/hub?scope=people&record=candidate-${adminCandidate.candidate_id}`,
    200,
    adminCookie,
  );
  await expectStatus(
    `/hub?scope=companies&record=company-${adminCompany.company_id}`,
    200,
    adminCookie,
  );
  await expectStatus(
    `/hub?scope=demand&record=request-${adminRequest.request_uuid}`,
    200,
    adminCookie,
  );
  await expectStatus(
    `/hub?scope=money&record=transfer-${adminTransfer.transfer_id}`,
    200,
    adminCookie,
  );
  await expectStatus(
    `/hub?scope=compliance&record=id-${idRequest.cir_uuid}`,
    200,
    adminCookie,
  );
  ...
  // App Router redirect() renders 200 with meta-refresh, not 307
  await expectStatus(
    `/admin/candidates/${adminCandidate.candidate_id}`,
    200,
    adminCookie,
  );
  await expectBodyIncludes(
    `/admin/candidates?candidate=${adminCandidate.candidate_id}&tabs=${adminCandidate.candidate_id}`,
    200,
    "Readiness",
    adminCookie,
  );
  ...
  await expectStatus(
    `/admin/companies/${adminCompany.company_id}`,
    200,
    adminCookie,
  );
  await expectStatus("/admin/requests", 200, adminCookie);
  await expectStatus(
    `/admin/requests/${adminRequest.request_uuid}`,
    200,
    adminCookie,
  );
  await expectBodyIncludes(
  ...
  await expectStatus(
    `/admin/transfers/${adminTransfer.transfer_id}`,
    200,
    adminCookie,
  );
  ...
  await expectStatus(
    `/hub?scope=demand&record=request-${staffRequest.request_uuid}`,
    200,
    staffCookie,
  );
  await expectStatus(
    `/hub?scope=people&record=candidate-${staffCandidate.candidate_id}`,
    200,
  ...
  await expectStatus(
    `/staff/requests/${staffRequest.request_uuid}`,
    200,
    staffCookie,
  );
  await expectBodyIncludes(
  ...
  await expectBodyIncludes(
    "/staff/candidates?view=assigned",
    200,
    "Assigned to me",
    staffCandidateCookie,
  );
  ...
  // App Router redirect() renders 200 with meta-refresh, not 307
  await expectStatus(
    `/staff/candidates/${staffCandidate.candidate_id}`,
    200,
    staffCandidateCookie,
  );
  await expectBodyIncludes(
    `/staff/candidates?candidate=${staffCandidate.candidate_id}&tabs=${staffCandidate.candidate_id}`,
    200,
    "Readiness",
    staffCandidateCookie,
  );
  ...
  await expectStatus(
    `/hub?scope=people&record=candidate-${candidateInvitation.candidate_id}`,
    200,
    candidateInvitationCookie,
  );
  ...
  await expectStatus(
    `/candidate/invitations/${candidateInvitation.invitation_uuid}`,
    200,
    candidateInvitationCookie,
  );
  await expectStatus("/candidate/work-logs", 200, candidateWorkLogCookie);
  await expectStatus(
    `/candidate/work-logs/${candidateWorkLog.candidate_working_hour_uuid}`,
    200,
    candidateWorkLogCookie,
  );

  await expectStatus("/hub", 200, companyCookie);
  await expectStatus(
    `/hub?scope=companies&record=company-${contactCompany.company_id}`,
    200,
    companyCookie,
  );
  await expectStatus(
    `/hub?scope=demand&record=request-${contactRequest.company.request[0].request_uuid}`,
    200,
    companyRequestCookie,
  );
  await expectStatus("/company", 200, companyCookie);
  await expectStatus("/company/companies", 200, companyCookie);
  await expectStatus(
    `/company/companies/${contactCompany.company_id}`,
    200,
    companyCookie,
  );
  await expectStatus("/company/requests", 200, companyRequestCookie);
  await expectStatus(
    `/company/requests/${contactRequest.company.request[0].request_uuid}`,
    200,
    companyRequestCookie,
  );
  ...
  await expectStatus(
    `/inspector/id-requests/${idRequest.cir_uuid}`,
    200,
    inspectorCookie,
  );
  ...
  await expectOneOf(
    `/candidate/invitations/${otherCandidateInvitation.invitation_uuid}`,
    [200, 404],
    candidateInvitationCookie,
  );
  await expectOneOf(
    `/candidate/work-logs/${otherCandidateWorkLog.candidate_working_hour_uuid}`,
    [200, 404],
    candidateWorkLogCookie,
  );
  await expectOneOf(
    `/company/companies/${otherContactCompany.company_id}`,
    [200, 404],
    companyCookie,
  );
  await expectOneOf(
    `/company/requests/${otherContactRequest.company.request[0].request_uuid}`,
    [200, 404],
    companyRequestCookie,
  );
  await expectStatus(
    `/staff/candidates/${unassignedStaffCandidate.candidate_id}`,
    200,
    staffCandidateCookie,
  );
  await expectBodyIncludes(
  ...
  await expectOneOf(
    `/staff/requests/${adminRequest.request_uuid}`,
    [200, 404],
    staffCookie,
  );
