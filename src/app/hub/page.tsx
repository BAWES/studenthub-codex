...
import { HubContent, type HubContentData } from "@/modules/hub/HubContent";
import { type HubCommand } from "@/modules/hub/HubShortcuts";
import { getUnifiedHubAction } from "@/modules/app/actions";
import { parseHubScope } from "@/modules/app/utils";
...
export default async function HubPage({
  ...
  const data: HubContentData = await getUnifiedHubAction({
    query: params.q,
    scope,
  ...
  return (
    <WorkspaceShell session={session} eyebrow="Hub" title="Command Center" metrics={[]}>
      <HubContent
        data={data}
        guide={guide}
        commands={commands}
...
/* ── helper: parse the `required` search param into a Role ── */
...
/* ── guide builder: one per role, keyed off live queue counts ── */
...
function buildRoleGuide(role: Role, data: HubContentData): RoleGuide {
  const queueValue = (label: string) =>
    data.queues.find((q) => q.label.toLowerCase().includes(label))?.value ?? 0;
  ...
  const activeCandidates =
    data.system.find((item) => item.label === "Active candidates")?.value ?? 0;
  const transfers =
    data.system.find((item) => item.label === "Transfers")?.value ?? 0;
  ...
  const guides: Record<Role, RoleGuide> = {
    admin: {
      ...
      journeys: [
        ...
        },
        {
          ...
          description:
            "Track time, candidate pay, employer charges, and invoice-ready transfer records.",
        ...
        },
        {
          ...
          title: "Compliance",
          description:
            "Keep identity and civil ID review separate from day-to-day placement work.",
    ...
    staff: {
      ...
      guardrail:
        "Staff only sees assigned requests and candidates connected to their staff account.",
      ...
      journeys: [
        {
          ...
          description:
            "Start with the employer request, then work candidates against that specific need.",
    ...
    candidate: {
      ...
      guardrail:
        "Candidate login only exposes the signed-in candidate profile, invitations, and work history.",
      ...
      journeys: [
        {
          ...
          description:
            "Finish the details employers and staff need before sending you to a role.",
        ...
        },
        {
          ...
          description:
            "See the roles and requests sent to your account.",
        ...
        },
        {
          ...
          description:
            "Check imported shifts, timer records, and work history connected to payment.",
    ...
    company: {
      title:
        "Request staff, review candidates, and keep hiring paperwork together.",
      description:
      ...
      guardrail:
        "Company login only exposes linked companies and requests for that company contact.",
      ...
      journeys: [
        {
          ...
          description:
            "Create and manage demand by company, position, seats, and status.",
        ...
        },
        {
          ...
          description:
            "Keep employer identity, contacts, stores, and hiring access clean.",
    ...
    inspector: {
      title:
        "Clear identity review without mixing it into placement work.",
      description:
        "Inspector accounts should move through civil ID batches, candidate lists, and document status quickly.",
      guardrail:
        "Inspector login only exposes ID review queues.",
      ...
      journeys: [
        {
          ...
          description:
            "Work each imported batch from candidate list to resolved document status.",
...
function buildCommands(data: HubContentData): HubCommand[] {
    ...
    const shortcut =
      item.href === "/app"
        ...
        : item.label.toLowerCase().includes("request")
          ? "G R"
          : item.label.toLowerCase().includes("candidate") ||
              item.label.toLowerCase().includes("company")
            ? "G C"
            : undefined;
    commands.push({
      id: `nav-${item.href}`,
  ...
  for (const scope of data.scopes) {
    ...
    commands.push({
      ...
      subtitle: `Scope results to ${scope.label}`,
      section: "Search scopes",
      href: `/app?${params.toString()}` as Route,
    });
  }
