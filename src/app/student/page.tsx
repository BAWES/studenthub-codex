...
function AvatarCircle({ name }: { name?: string | null }) {
  ...
  return (
    <div className="flex size-24 items-center justify-center rounded-full bg-[#fef1ef] text-coral text-3xl font-bold shrink-0">
      {initial}
    </div>
  );
}
        ...
        <div className="mx-auto max-w-lg text-center">
          <Card>
            <CardContent className="flex flex-col items-center gap-4 py-12">
              <div className="flex size-16 items-center justify-center rounded-full bg-[#fef1ef] text-coral text-2xl">
                !
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {profile.skills.map((skill) => (
                  <Badge
                    key={skill.id}
                    variant="secondary"
                    className="rounded-full bg-[#fef1ef] border border-[rgba(235,102,81,0.2)] text-coral font-bold"
                  >
                    {skill.name}
                  </Badge>
                ))}
