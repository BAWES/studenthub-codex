...
import { buildIdCardHtml } from "@/modules/candidate-id-cards/pdf-helpers";
import { getIdCardPdfData } from "@/modules/candidate-id-cards/pdf-actions";
...
export async function GET(
  ...
  try {
    ...
    if (!data) {
      return new NextResponse("Candidate not found or no active ID card", { status: 404 });
    }
    ...
    return new NextResponse("Failed to generate the ID card PDF", { status: 500 });
  }
}
    ...
    const pdfBuffer = await page.pdf({
      ...
      footerTemplate: `
        <div style="font-size:10px;color:#aaa;text-align:center;width:100%;padding:0 15mm;">
          Candidate ID Card &mdash; Page <span class="pageNumber"></span> of <span class="totalPages"></span>
        </div>`,
    });
