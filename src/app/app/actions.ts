...
// Colocated replacement for src/modules/hub/data.ts
// ---------------------------------------------------------------------------
...
const hubScopes = Object.entries(scopeLabels).map(([value, label]) => ({
...
async function canStaffAccessCandidate(
  staffId: number,
  candidateId: number,
) {
...
async function buildPreview(
  ...
  return {
    ...
    href: result.href,
    actions: result.href
      ? [{ label: "Open full record", href: result.href }]
      : [],
    flags: [result.type],
...
async function getCandidatePreview(
  session: SessionUser,
  candidateId: number,
): Promise<HubPreview | null> {
  if (session.role === "candidate" && Number(session.id) !== candidateId)
  ...
  if (
    session.role === "staff" &&
    !(await canStaffAccessCandidate(Number(session.id), candidateId))
  )
    return null;

  const [candidate, notes, invitations, histories] =
    await prisma.$transaction([
      ...
      prisma.candidate_work_history.findMany({
        ...
        select: {
          ...
          start_date: true,
          end_date: true,
          company_candidate_work_history_company_idTocompany: {
  ...
  const actions = [
    ...
    candidate.candidate_email
      ? { label: "Email", href: `mailto:${candidate.candidate_email}` }
      : null,
    candidate.candidate_phone
      ? { label: "Call", href: `tel:${candidate.candidate_phone}` }
      : null,
  ].filter(
    (action): action is { label: string; href: string } => Boolean(action),
  );
  ...
  return {
    ...
    facts: [
      ...
      {
        label: "University",
        value: candidate.university?.university_name_en ?? "Not set",
      },
      {
        label: "Assignment",
        value:
    ...
    related: [
      ...
      },
      {
        ...
        rows: invitations.map((invitation) => ({
          ...
          title:
            invitation.request.request_position_title ?? "Invitation",
          subtitle:
            invitation.request.company?.company_name ?? "No company",
...
async function getCompanyPreview(
  ...
  if (session.role === "company") {
    const ids = await companyIdsForContact(session.id);
    if (!ids.includes(companyId)) return null;
  }

  const [company, requests, contacts, stores, notes] =
    await prisma.$transaction([
  ...
  const actions = [
    ...
    company.company_email
      ? { label: "Email", href: `mailto:${company.company_email}` }
      : null,
    company.company_website
      ? { label: "Website", href: company.company_website }
      : null,
  ].filter(
    (action): action is { label: string; href: string } => Boolean(action),
  );
  ...
  return {
    ...
    facts: [
      ...
      {
        label: "Common name",
        value: company.company_common_name_en ?? "Not set",
      },
      {
        label: "Owner",
        value: company.staff?.staff_name ?? "Unassigned",
      },
      {
        label: "Rate",
        value: formatMoney(
      ...
      {
        label: "Country",
        value: company.country?.country_name_en ?? "No country",
      },
...
async function getRequestPreview(
  ...
  if (session.role === "candidate" || session.role === "inspector")
    return null;

  const companyIds =
    session.role === "company"
      ? await companyIdsForContact(session.id)
      : [];
  ...
  const [request, applications, interviews, invitations, activities, notes, stories] =
    await prisma.$transaction([
  ...
  const companyLink = request.company?.company_id
    ? companyHref(session, request.company.company_id)
    : undefined;
  const actions = [
    href ? { label: "Open full record", href } : null,
    ...
    request.contact?.contact_email
      ? {
          label: "Email contact",
          href: `mailto:${request.contact.contact_email}`,
        }
      : null,
  ].filter(
    (action): action is { label: string; href: string } => Boolean(action),
  ...
  return {
    ...
    facts: [
      ...
      {
        label: "Compensation",
        value: request.request_compensation || "Not set",
      },
      {
        label: "Location",
        value: request.request_location ?? "Not set",
      },
      {
        label: "Owner",
        value: request.staff?.staff_name ?? "Unassigned",
      },
      {
        label: "Contact",
        value: request.contact?.contact_name ?? "No contact",
      },
      {
        label: "Updated",
        value: formatDate(request.request_updated_datetime),
      },
    ],
    related: [
      {
        ...
        rows: applications.map((application) => ({
          ...
          title:
            application.candidate?.candidate_name ?? "Unknown candidate",
          subtitle:
            application.candidate?.candidate_email ?? "No email",
          meta: `Status ${application.status ?? 0} · ${formatDate(application.created_at)}`,
          href: application.candidate?.candidate_id
            ? candidateHref(session, application.candidate.candidate_id)
            : undefined,
        })),
      },
      {
        ...
        rows: interviews.map((interview) => ({
          ...
          title:
            interview.candidate?.candidate_name ?? "Interview",
          subtitle:
            interview.candidate?.candidate_email ?? "No email",
          meta: `Status ${interview.status ?? 0} · ${formatDate(interview.interview_at)}`,
          href: interview.candidate?.candidate_id
            ? candidateHref(session, interview.candidate.candidate_id)
            : undefined,
        })),
      },
      {
        ...
        rows: invitations.map((invitation) => ({
          id: invitation.invitation_uuid,
          title:
            invitation.candidate?.candidate_name ?? "Invitation",
          subtitle:
            invitation.candidate?.candidate_email ?? "No email",
          meta: `Status ${invitation.invitation_status ?? 0} · ${formatDate(invitation.invitation_created_at)}`,
          href: invitation.candidate?.candidate_id
            ? candidateHref(session, invitation.candidate.candidate_id)
            : undefined,
        })),
      },
      {
...
async function getTransferPreview(
  ...
  const [transfer, candidates, invoices, fileEntries] =
    await prisma.$transaction([
      prisma.transfer.findFirst({
        ...
        select: {
          ...
          currency_code: true,
          company: {
            select: { company_name: true, company_email: true },
          },
          staff_transfer_transfer_created_byTostaff: {
  ...
  return {
    ...
    facts: [
      ...
      {
        label: "Total",
        value: formatMoney(total, transfer.currency_code ?? "KWD"),
      },
      {
        label: "Cost",
        value: formatMoney(
      ...
      {
        label: "Payment",
        value: transfer.payment_received_on
          ? formatDate(transfer.payment_received_on)
          : "Not received",
      },
      {
        label: "Created by",
        value:
          transfer.staff_transfer_transfer_created_byTostaff?.staff_name ??
          "Unknown",
      },
      {
        label: "Updated",
        value: formatDate(transfer.transfer_updated_at),
      },
    ],
    related: [
      {
        ...
        rows: candidates.map((candidate) => ({
          id: candidate.tc_id,
          title:
            candidate.candidate?.candidate_name ?? "Unknown candidate",
          subtitle:
            candidate.store?.store_name ??
          ...
          href: candidate.candidate?.candidate_id
            ? candidateHref(session, candidate.candidate.candidate_id)
            : undefined,
        })),
      },
      {
...
async function getIdRequestPreview(
  session: SessionUser,
  requestUuid: string,
): Promise<HubPreview | null> {
  ...
  const candidateIds = parseCandidateIds(request.candidate_ids).slice(
    0,
    8,
  );
  ...
  return {
    ...
    flags: [
      ...
      candidates.some(
        (candidate) => candidate.candidate_civil_need_verification,
      )
        ? "Verification required"
        : "No active verification flag",
    ],
    facts: [
      {
        label: "Batch UUID",
        value: request.cir_uuid.slice(0, 18),
      },
      {
        label: "Candidates",
        value: parseCandidateIds(request.candidate_ids).length,
      },
      {
        label: "Created by",
        value:
          request.staff_candidate_id_request_created_byTostaff
            ?.staff_name ?? "Unknown",
      },
      {
        label: "Updated by",
        value:
          request.staff_candidate_id_request_updated_byTostaff
            ?.staff_name ?? "Unknown",
      },
