...
export default async function ProfileWorkLogsPage() {
  ...
  return (
    <WorkspaceShell session={session} eyebrow="Profile" title="Work Logs" metrics={[]}>
      <DataTable
        title="Work Log History"
        description="Imported shifts, timer entries, and status values connected to your candidate account."
        rows={rows}
        rowHref={(row) => `/app/profile/work-logs/${row.id}` as Route}
        columns={[
