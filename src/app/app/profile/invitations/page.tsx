...
export default async function ProfileInvitationsPage() {
  const session = await requireRoleCapability("candidate", "candidate.read.own");
  const rows = await getCandidateInvitationRows(Number(session.id));

  return (
    <WorkspaceShell session={session} eyebrow="Profile" title="Invitations" metrics={[]}>
      <DataTable
        title="Invitation History"
        description="Requests and roles sent to your candidate account from the imported production data."
        rows={rows}
        rowHref={(row) => `/app/profile/invitations/${row.id}` as Route}
        columns={[
