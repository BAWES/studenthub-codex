...
export function CandidateChatClient({ session, conversations }: Props) {
  ...
  return (
    <div className="flex flex-1 overflow-hidden rounded-lg border border-[#e8e6e3] bg-white shadow-sm">
      {/* Conversation list — Slack-style left sidebar */}
      <div className="w-72 shrink-0 overflow-y-auto bg-[#f4f2ef] border-r border-[#e8e6e3]">
        {/* Sidebar header */}
        <div className="px-4 py-3 border-b border-[#e8e6e3]">
          <h2 className="text-sm font-semibold text-[#1d1c1a] uppercase tracking-wide">
            Conversations
          </h2>
                ...
                <li key={conv.chat_uuid} className="px-1">
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 rounded-md transition-colors ${
                      isActive
                        ? "bg-[#fef1ef] border-l-2 border-[#eb6651]"
                        : "hover:bg-[#e8e6e3] border-l-2 border-transparent"
                    }`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span
                        className={`text-sm truncate ${
                          isActive ? "font-semibold text-[#1d1c1a]" : "font-medium text-[#1d1c1a]"
                        }`}
                      >
                        {label}
                      </span>
                    ...
                    <span className="text-xs text-[#6e6b66] block mt-0.5">
                      {conv.company_name ? (
                        <>
                          {conv.store_name ? (
        ...
        {!selectedConv ? (
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center max-w-xs">
              ...
              <p className="text-sm font-medium text-[#1d1c1a]">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center max-w-xs">
              ...
              <p className="text-xs text-[#6e6b66] mt-1">{error}</p>
              <button
                type="button"
                onClick={() => selectedConv && selectConversation(selectedConv)}
                className="mt-3 inline-flex items-center gap-1.5 rounded-md bg-[#eb6651] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#d45441] transition-colors"
              >
                Retry
              </button>
            </div>
          ...
          <>
            {/* Conversation header */}
            <div className="px-4 py-3 border-b border-[#e8e6e3] bg-white shrink-0">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-[#eb6651] flex items-center justify-center text-white text-sm font-bold shrink-0">
                  {getConversationLabel(selectedConv).charAt(0).toUpperCase()}
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#1d1c1a]">
                    ...
                    >
                      <div
                        className={`max-w-[70%] rounded-lg px-4 py-3 ${
                          isFromCandidate
                            ? "bg-[#eb6651] text-white rounded-br-sm"
                            : "bg-white border border-[#e8e6e3] text-[#1d1c1a] rounded-bl-sm shadow-sm"
                        }`}
                      >
                        ...
                        <div
                          className={`flex items-center justify-end gap-1.5 mt-1.5 ${
                            isFromCandidate ? "text-white/70" : "text-[#a09d98]"
                          }`}
                        >
            ...
            {/* Message input bar */}
            <div className="shrink-0 border-t border-[#e8e6e3] bg-white px-4 py-3">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  rows={1}
                  className="flex-1 resize-none rounded-lg border border-[#e8e6e3] bg-white px-3 py-2.5 text-sm text-[#1d1c1a] placeholder:text-[#a09d98] focus:outline-none focus:ring-2 focus:ring-[#eb6651] focus:border-transparent transition-all"
                  disabled={sending}
                />
                <button
                  type="button"
                  ...
                  className="inline-flex items-center justify-center rounded-lg bg-[#eb6651] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#d45441] disabled:opacity-50 disabled:cursor-not-allowed transition-colors shrink-0"
                >
                  {sending ? (
