...
import {
  ...
  type ListBusinessDevelopmentResult,
  type BusinessDevelopmentItem,
  type BusinessDevelopmentActionResult,
} from "./schemas";
...
export async function listBusinessDevelopment(
...
): Promise<ListBusinessDevelopmentResult> {
  ...
  const parsed = listBusinessDevelopmentSchema.safeParse(raw);
  if (!parsed.success) {
  ...
  // Find contact records matching the candidate's email
...
/**
 * Soft-delete (or cancel) a business development record.
 * Since company_request uses a boolean status field, we mark it as cancelled.
 */
