...
import { updateCandidateCertification } from "../actions";
...
export function CertificationEditForm({
  ...
  const [certificationName, setCertificationName] = useState(currentName);
  const [issuingOrganization, setIssuingOrganization] = useState(currentIssuer);
  ...
  const [credentialUrl, setCredentialUrl] = useState(currentCredentialUrl ?? "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result: CertificationActionResult = await updateCandidateCertification({
      certificationId,
      certificationName,
      issuingOrganization,
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button
          type="button"
          variant="outline"
          onClick={() => router.push(`/candidate/certifications/${certificationId}`)}
        >
          Cancel
        </Button>
      </div>
