        ...
        {results && (
          <div className="mb-4 flex items-center justify-between">
            <span className="text-xs font-semibold text-foreground">
              {isTyping ? (
                <>
                  <span className="bg-primary/70 mr-1.5 inline-block h-2 w-2 animate-pulse rounded-full align-middle" />
                  Searching...
                </>
              ) : (
                <>
            ...
            {!isTyping && (
              <Badge variant="secondary">{results.source.current}</Badge>
            )}
          </div>
        )}
                    ...
                    <CardContent className="p-0">
                      ...
                      <div className="mb-2 flex items-start justify-between gap-3">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {row.status && (
                            <Badge variant={genericStatusVariant(row.status) as BadgeProps["variant"]}>
                              {row.status}
                            </Badge>
                          )}
                        </div>
                ...
                {Array.from({ length: Math.min(totalPages, 5) }).map((_, i) => {
                  ...
                  return (
                    <Button
                      ...
                      size="sm"
                      className={cn(
                        p === page && "bg-[var(--sh-coral)] text-white hover:bg-[var(--sh-coral-hover)]",
                      )}
                      onClick={() => goToPage(p)}
                    >
                      {p}
