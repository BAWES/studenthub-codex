...
export function InvoiceDetailDrawer({
  ...
  return (
    <>
      ...
      {/* Drawer panel */}
      <div
        className="fixed top-0 right-0 z-50 h-full w-full max-w-lg overflow-y-auto border-l shadow-xl transition-transform duration-300 bg-[var(--surface)] border-[var(--border)]"
        style={{
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              ...
              <button
                onClick={onClose}
                className="h-10 rounded-lg px-4 text-sm font-semibold bg-[var(--sh-info)] text-white"
              >
                Close
              </button>
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {detail.metrics.map((m) => (
                      ...
                      >
                        ...
                        <p className="text-lg font-bold mt-1 text-[var(--accent)]">{m.value}</p>
                        {m.note && (
