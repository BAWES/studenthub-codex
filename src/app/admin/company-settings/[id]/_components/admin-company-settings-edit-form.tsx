    ...
    <section className="mt-6">
      {!expanded ? (
        <button
          ...
          className="rounded-lg px-4 py-2 text-sm font-semibold bg-[var(--sh-primary)] text-white"
        >
          Edit settings
        </button>
      ) : (
        <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5">
          ...
          >
              ...
              return (
                <div className="grid gap-1" key={name}>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="rounded-lg px-3 py-2 text-sm border resize-y bg-[var(--surface)] border-[var(--border)] text-[var(--ink)]"
                  />
                </div>
              );
            })}
            ...
            {[
              ...
              <div className="grid gap-1" key={field.name}>
                <label className="text-xs font-medium text-[var(--muted)]">{field.label}</label>
                <select
                  name={field.name}
            ...
            <div className="grid gap-1">
              <label className="text-xs font-medium text-[var(--muted)]">Currency code</label>
              <input
                name="currencyCode"
            ...
            <div className="col-span-full flex items-center gap-3 pt-2">
              ...
              <button
                ...
                className="h-9 rounded-lg px-4 text-sm text-[var(--muted)]"
              >
                Cancel
              </button>
              {state?.error ? (
                <p className="text-xs text-[var(--sh-error)]">{state.error}</p>
              ) : null}
            </div>
          </form>
        </div>
