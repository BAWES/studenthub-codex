...
import { getEventTimeline } from "@/modules/admin/event/actions";
...
export function AdminEventTable({ session, events }: Props) {
  ...
  return (
    ...
    >
      {error ? (
        <div className="mb-4 rounded-lg border border-destructive/50 px-4 py-3 text-sm text-destructive bg-destructive/10">
          {error}
        </div>
      ) : null}
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="text-xs px-3 py-1 rounded-md border bg-surface text-muted-foreground hover:opacity-80 transition-opacity"
            >
              Close timeline
            </button>
          </div>
                  ...
                  <div className="space-y-2">
                    {entry.events.map((evt) => (
                      <div
                        ...
                        className="rounded-lg border p-3 text-sm bg-surface"
                      >
      ...
      <DataTable
        ...
        rowHref={(row) => `/admin/event/${row.id}` as Route}
        columns={[
          {
            key: "activity_uuid",
            label: "UUID",
          ...
          },
          {
            ...
            render: (row) => (
              <button
                ...
                className="text-sm font-mono text-accent hover:opacity-80 transition-opacity"
