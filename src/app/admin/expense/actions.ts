...
export async function getExpenseDetail(expenseUuid: string) {
  ...
  const expense = await prisma.expense.findUnique({
    ...
    select: {
      ...
      created_at: true,
      updated_at: true,
      admin_expense_created_byToadmin: { select: { admin_id: true, admin_name: true } },
      admin_expense_updated_byToadmin: { select: { admin_id: true, admin_name: true } }
    }
  });
...
export async function updateExpense(
  ...
  await prisma.expense.update({
    ...
    data: {
      ...
      transaction_datetime: data.transaction_datetime ? new Date(data.transaction_datetime) : null,
      updated_at: new Date()
    }
  });
