...
// ---------------------------------------------------------------------------
// Admin Bank — server actions
// ---------------------------------------------------------------------------
// Ported from Yii2 admin/modules/v1/controllers/BankController.php
//
// Actions:
//   - listBanks        — list all banks with pagination + search
//   - getBank          — single bank detail with candidate count
//   - createBank       — create a new bank
//   - updateBank       — update a bank
//   - deleteBank       — soft-delete a bank
//
// Legacy Bank fields:
//   bank_name, bank_iban_code, bank_swift_code, bank_code_abk,
//   bank_address, bank_transfer_type
// ---------------------------------------------------------------------------
...
export async function listBanks(
  ...
  const result = {
    items: banks.map((b): BankRow => ({
      ...
      created_at: null, // bank model has no created_at field
    })),
    total,
    page,
    limit,
