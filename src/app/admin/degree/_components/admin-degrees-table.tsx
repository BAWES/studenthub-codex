"use client";

import { useActionState, useState, useRef } from "react";
import { useRouter } from "next/navigation";
import { DataTable } from "@/modules/workspace/DataTable";
import { WorkspaceShell } from "@/modules/workspace/WorkspaceShell";

import type { SessionUser } from "@/modules/auth/types";
import type { DegreeItem } from "../schemas";
import { createDegree, updateDegree, deleteDegree } from "../actions";

type Props = {
  session: SessionUser;
  degrees: DegreeItem[];
};

export function AdminDegreesTable({ session, degrees }: Props) {
  const router = useRouter();
  const [editingId, setEditingId] = useState<string | null>(null);

  return (
    <WorkspaceShell
      session={session}
      eyebrow="Admin settings"
      title="Manage degrees — academic degree programs offered across institutions."
      metrics={[
        { label: "Total degrees", value: degrees.length, note: "Degrees in the system" },
      ]}
    >
      <section className="mb-6">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5">
          <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--ink)" }}>Add degree</h3>
          <CreateDegreeForm onSuccess={() => router.refresh()} />
        </div>
      </section>

      <DataTable
        title="Degrees"
        description="All academic degrees. Click a degree name to edit or delete."
        rows={degrees.map((d) => ({ ...d, id: d.degree_uuid }))}
        rowHref={undefined}
        columns={[
          {
            key: "name",
            label: "Name (EN)",
            render: (row) =>
              editingId === row.degree_uuid ? (
                <EditDegreeForm
                  row={row}
                  onDone={() => { setEditingId(null); router.refresh(); }}
                  onCancel={() => setEditingId(null)}
                />
              ) : (
                <button
                  type="button"
                  className="text-sm hover:underline"
                  style={{ color: "var(--sh-primary)" }}
                  onClick={() => setEditingId(row.degree_uuid)}
                >
                  {row.degree_name_en}
                </button>
              ),
          },
          {
            key: "name_ar",
            label: "Name (AR)",
            render: (row) =>
              editingId === row.degree_uuid ? null : (
                <span className="text-sm" style={{ color: "var(--ink)" }}>
                  {row.degree_name_ar ?? "—"}
                </span>
              ),
          },
          {
            key: "sort_order",
            label: "Sort",
            render: (row) =>
              editingId === row.degree_uuid ? null : (
                <span className="text-sm" style={{ color: "var(--muted)" }}>
                  {row.degree_sort_order ?? "—"}
                </span>
              ),
          },
          {
            key: "updated",
            label: "Updated",
            render: (row) =>
              editingId === row.degree_uuid ? null : (
                <span className="text-sm" style={{ color: "var(--muted)" }}>
                  {row.degree_updated_at
                    ? new Date(row.degree_updated_at).toLocaleDateString()
                    : "—"}
                </span>
              ),
          },
          {
            key: "actions",
            label: "",
            render: (row) =>
              editingId !== row.degree_uuid ? (
                <button
                  type="button"
                  className="text-xs px-2 py-1 rounded hover:bg-red-500/10"
                  style={{ color: "var(--sh-error)" }}
                  onClick={async () => {
                    if (confirm(`Delete degree "${row.degree_name_en}"?`)) {
                      const result = await deleteDegree(row.degree_uuid);
                      if (result.operation === "error") {
                        alert(result.message);
                      }
                      router.refresh();
                    }
                  }}
                >
                  Delete
                </button>
              ) : null,
          },
        ]}
      />
    </WorkspaceShell>
  );
}

function CreateDegreeForm({ onSuccess }: { onSuccess: () => void }) {
  const [state, action, pending] = useActionState(
    async (_prev: { error?: string } | null, formData: FormData) => {
      const nameEn = formData.get("degreeNameEn") as string;
      const nameAr = formData.get("degreeNameAr") as string;
      const sortOrder = formData.get("degreeSortOrder") as string;

      const result = await createDegree(
        nameEn,
        nameAr || undefined,
        undefined,
        sortOrder ? Number(sortOrder) : undefined,
      );
      if (result.operation === "success") {
        onSuccess();
        return { error: undefined };
      }
      return { error: result.message };
    },
    null,
  );
  const formRef = useRef<HTMLFormElement>(null);

  return (
    <form
      ref={formRef}
      action={action}
      className="flex flex-wrap items-end gap-3"
      onSubmit={() => setTimeout(() => { formRef.current?.reset(); }, 100)}
    >
      <div className="grid gap-1">
        <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>Name (EN) *</label>
        <input name="degreeNameEn" required maxLength={255} placeholder="e.g. Bachelor of Science"
          className="h-9 rounded-lg px-3 text-sm border w-48"
          style={{ background: "var(--surface)", borderColor: "var(--border)", color: "var(--ink)" }} />
      </div>
      <div className="grid gap-1">
        <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>Name (AR)</label>
        <input name="degreeNameAr" maxLength={255} placeholder="بكالوريوس"
          className="h-9 rounded-lg px-3 text-sm border w-36"
          style={{ background: "var(--surface)", borderColor: "var(--border)", color: "var(--ink)" }} />
      </div>
      <div className="grid gap-1">
        <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>Sort order</label>
        <input name="degreeSortOrder" type="number" placeholder="0"
          className="h-9 rounded-lg px-3 text-sm border w-20"
          style={{ background: "var(--surface)", borderColor: "var(--border)", color: "var(--ink)" }} />
      </div>
      <button
        type="submit" disabled={pending}
        className="h-9 rounded-lg px-4 text-sm font-semibold"
        style={{ background: "var(--sh-primary)", color: "#fff" }}
      >
        {pending ? "Adding..." : "Add"}
      </button>
      {state?.error ? (
        <p className="text-xs w-full" style={{ color: "var(--sh-error)" }}>{state.error}</p>
      ) : null}
    </form>
  );
}

function EditDegreeForm({
  row, onDone, onCancel,
}: {
  row: DegreeItem;
  onDone: () => void;
  onCancel: () => void;
}) {
  const [state, action, pending] = useActionState(
    async (_prev: { error?: string } | null, formData: FormData) => {
      const nameEn = formData.get("degreeNameEn") as string;
      const nameAr = formData.get("degreeNameAr") as string;
      const sortOrder = formData.get("degreeSortOrder") as string;

      const result = await updateDegree(
        row.degree_uuid, nameEn,
        nameAr || undefined, undefined,
        sortOrder ? Number(sortOrder) : undefined,
      );
      if (result.operation === "success") {
        onDone();
        return { error: undefined };
      }
      return { error: result.message };
    },
    null,
  );

  return (
    <form action={action} className="flex items-center gap-2 flex-wrap">
      <input name="degreeNameEn" defaultValue={row.degree_name_en} required maxLength={255}
        className="h-8 rounded px-2 text-sm border w-40"
        style={{ background: "var(--surface)", borderColor: "var(--border)", color: "var(--ink)" }} />
      <input name="degreeNameAr" defaultValue={row.degree_name_ar ?? ""} maxLength={255}
        className="h-8 rounded px-2 text-sm border w-36"
        style={{ background: "var(--surface)", borderColor: "var(--border)", color: "var(--ink)" }} />
      <input name="degreeSortOrder" type="number" defaultValue={row.degree_sort_order ?? ""}
        className="h-8 rounded px-2 text-sm border w-16"
        style={{ background: "var(--surface)", borderColor: "var(--border)", color: "var(--ink)" }} />
      <button type="submit" disabled={pending}
        className="h-8 rounded px-3 text-xs font-semibold"
        style={{ background: "var(--sh-primary)", color: "#fff" }}>
        {pending ? "..." : "Save"}
      </button>
      <button type="button" onClick={onCancel}
        className="h-8 rounded px-3 text-xs" style={{ color: "var(--muted)" }}>
        Cancel
      </button>
      {state?.error ? (
        <p className="text-xs w-full" style={{ color: "var(--sh-error)" }}>{state.error}</p>
      ) : null}
    </form>
  );
}
