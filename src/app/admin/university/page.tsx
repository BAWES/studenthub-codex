...
export default async function AdminUniversityPage() {
  ...
  return (
    <WorkspaceShell session={session} eyebrow="Admin" title="Universities" metrics={[]}>
      <DataTable
        title="Universities"
        description="Manage university records across the platform."
        rows={rows}
        rowHref={(row) => `/admin/university/${row.id}` as Route}
        columns={[
          { key: "name_en", label: "Name (English)", render: (row) => <strong>{row.name_en}</strong> },
          { key: "name_ar", label: "Name (Arabic)", render: (row) => row.name_ar },
