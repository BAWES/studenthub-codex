...
describe("getAllAgentsHealth", () => {
  ...
  it("returns health data for a single healthy agent", async () => {
    mockClient.query
      .mockResolvedValueOnce({ rows: [{ id: "agt-1", name: "Coder", status: "running", role: "software_engineer" }] })
  ...
  it("handles edge case: zero runs", async () => {
    mockClient.query
      .mockResolvedValueOnce({ rows: [{ id: "agt-4", name: "Zero", status: "idle", role: "test" }] })
...
describe("agentsHealthDataSchema (output validation)", () => {
  ...
  it("rejects invalid agent inside array", () => {
    const r = agentsHealthDataSchema.safeParse({
      agents: [{ id: "agt-1", name: "", status: "running", role: "test", heartbeatMetrics: [], lastHeartbeat: null, issuesDone: 0, issuesInProgress: 0 }],
    });
    expect(r.success).toBe(false);
  });
