...
export async function deleteDegreeGroup(groupUuid: string) {
  await requireRoleCapability("admin", "admin.system");

  // Check if any degrees reference this group
  ...
  if (degreeCount > 0) {
    throw new Error(
      `Cannot delete degree group: ${degreeCount} degree(s) still reference it. Unassign them first.`
    );
  }
