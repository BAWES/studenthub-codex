...
export default async function AdminDegreeGroupPage() {
  ...
  return (
    <WorkspaceShell session={session} eyebrow="Admin" title="Degree Groups" metrics={[]}>
      <DataTable
        title="Degree Groups"
        description="Manage degree groups across the platform"
        rows={rows}
        ...
        columns={[
          ...
          { key: "skip_major", label: "Skip Major", render: (row) => row.skip_major === 1 ? "Yes" : "No" },
          { key: "degree_count", label: "Degrees", render: (row) => row.degree_count },
