...
const STATUS_STYLES: Record<
  number,
  { btn: string; dot: string }
> = {
  0: { btn: "bg-blue-500/10 text-blue-600", dot: "bg-blue-600" },
  1: { btn: "bg-amber-500/10 text-amber-700", dot: "bg-amber-700" },
  2: { btn: "bg-green-500/10 text-green-700", dot: "bg-green-700" },
  3: { btn: "bg-gray-500/10 text-gray-600", dot: "bg-gray-600" },
};
        ...
        columns={[
          ...
          },
          {
            ...
            render: (row) => {
              ...
              const styles = STATUS_STYLES[status] ?? STATUS_STYLES[0];
              return (
                <button
                  type="button"
                  onClick={() => toggleStatus(row)}
                  className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-all duration-200 hover:opacity-80 ${styles.btn}`}
                >
                  <span
