...
export const createStoreSchema = z.object({
  ...
  store_location: z.string().max(255).optional().default(""),
  company_id: z.coerce.number().int().positive().optional(),
  store_status: z.coerce.number().int().min(0).optional().default(10),
});

export const updateStoreSchema = z.object({
  storeId: z.coerce.number().int().positive("Store ID is required"),
  store_name: z.string().min(1).max(255).optional(),
  store_location: z.string().max(255).optional(),
  company_id: z.coerce.number().int().positive().optional(),
  store_status: z.coerce.number().int().min(0).optional(),
});
