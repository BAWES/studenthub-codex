  ...
  return (
    <div className="rounded-lg border border-[var(--border)] bg-white overflow-hidden">
      ...
      {!loading && !error && total > 0 && onPageChange && (
        <div className="flex items-center justify-between px-4 py-3 text-sm border-t border-[rgba(255,255,255,0.06)] text-[var(--muted)]">
          <span>Showing {1 + (page - 1) * 20}-{Math.min(page * 20, total)} of {total}</span>
          <div className="flex items-center gap-2">
            <button
              disabled={page <= 1}
              onClick={() => onPageChange(page - 1)}
              className="px-3 py-1.5 rounded-md text-sm font-medium disabled:opacity-30 bg-[rgba(255,255,255,0.06)]"
