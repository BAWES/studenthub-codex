...
describe("companySearchResultSchema", () => {
  ...
  it("rejects non-integer page", () => {
    expect(
      companySearchResultSchema.safeParse({ ...validResult, page: 1.5 }).success
    ).toBe(false);
  });

  it("rejects non-integer matchingCount", () => {
    expect(
      companySearchResultSchema.safeParse({ ...validResult, matchingCount: 42.5 }).success
    ).toBe(false);
  });

  it("rejects non-array rows", () => {
    expect(
      companySearchResultSchema.safeParse({ ...validResult, rows: "not-array" }).success
    ).toBe(false);
  });

  it("rejects non-array facets", () => {
    expect(
      companySearchResultSchema.safeParse({ ...validResult, facets: "not-array" }).success
    ).toBe(false);
  });
});
