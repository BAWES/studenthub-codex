...
export function WorkspaceShellSkeleton({ rowCount = 8 }: { rowCount?: number }) {
  return (
    <div className="min-w-0 overflow-x-hidden grid content-start gap-3.5 p-3.5">
      {/* Topbar */}
      <section className="grid grid-cols-[1fr_minmax(220px,300px)] items-center gap-4.5 border border-border rounded-lg bg-card p-4">
        ...
        <div className="grid gap-1.5 p-3.5 border border-border rounded-lg bg-card">
          ...
          <Card key={col}>
            ...
            <div className="grid">
              {Array.from({ length: rowCount }).map((_, i) => (
                <article
                  key={i}
                  className="min-h-[72px] grid grid-cols-[1fr_minmax(100px,auto)] gap-4 px-4 py-3 border-b border-border last:border-b-0"
                >
  ...
  return (
    <div className="grid gap-3">
      ...
      <Card>
        <div className="grid">
          {/* Header row */}
          <div className="grid grid-cols-[1fr_1fr_120px_100px] gap-3 px-4 py-3 border-b border-border">
          ...
          {Array.from({ length: rows }).map((_, i) => (
            <div
              key={i}
              className="grid grid-cols-[1fr_1fr_120px_100px] gap-3 px-4 py-3 border-b border-border last:border-b-0"
            >
