  ...
  return (
    <main className="shell shellEmbedded">
      <section className="workspaceStage">
        ...
        {/* Content area */}
        <div className="px-5.5 py-[18px] grid gap-3.5">
...
export function DataTableSkeleton({ rows = 10 }: { rows?: number }) {
  return (
    <div className="px-5.5 py-[18px] grid gap-3">
      {/* Header */}
      ...
      <div className="grid gap-px">
        ...
        {Array.from({ length: rows }).map((_, i) => (
          <div
            key={i}
            className="grid grid-cols-[1fr_1fr_120px_100px] gap-3 px-3.5 py-3 border-t border-border"
          >
...
export function DetailPageSkeleton({ panels = 3 }: { panels?: number }) {
  return (
    <div className="px-5.5 py-[18px] grid gap-3.5">
      ...
      {/* Fact panels */}
      <div className={`grid gap-3 ${Math.min(panels, 2) >= 2 ? "grid-cols-2" : ""}`}>
...
export function QuickSkeleton({ lines = 4 }: { lines?: number }) {
  return (
    <div className="px-5.5 py-[14px] grid gap-2">
