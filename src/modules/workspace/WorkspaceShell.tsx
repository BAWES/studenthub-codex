  ...
  const stage = (
    <section className="min-w-0 overflow-x-hidden grid content-start gap-3.5 p-3.5">
      <section className="grid grid-cols-[1fr_minmax(220px,300px)] items-center gap-4.5 border border-border rounded-lg bg-card p-4">
        <div>
          <p className="text-sh-coral text-xs font-bold uppercase tracking-normal mb-2.5">{eyebrow}</p>
        ...
        <div className="min-w-0 grid gap-1.5 p-3.5 border border-border rounded-lg bg-card">
          <span className="text-sh-coral text-xs font-bold uppercase">{session.role}</span>
  ...
  return (
    <Card className="min-h-[360px]">
      <div className="flex items-center justify-between gap-4 px-4 py-3.5 border-b border-border">
        ...
        <span className="min-w-[30px] min-h-[30px] inline-flex items-center justify-center text-sh-coral border border-sh-coral/20 bg-sh-coral/5 font-bold text-sm rounded">{count}</span>
      </div>
      <div className="grid">
        {rows.length ? (
          rows.map((row) => (
