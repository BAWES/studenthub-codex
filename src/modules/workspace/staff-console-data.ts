// ---------------------------------------------------------------------------
// Staff Candidate Console — data functions
// Extracted from workspace/data/staff/candidates.ts after its consumers were
// migrated to colocated action files.
// ---------------------------------------------------------------------------
...
export async function getStaffCandidateConsole({
  ...
  const [fallbackCandidates, fallbackCompanies, fallbackTransfers, fallbackInvoices, fallbackIdRequests] =
    candidateIds.length
      ? await prisma.$transaction([
  ...
  const quiet = rows.filter((row) => row.priorityScore === 0 && row.candidateStatus !== 10).slice(0, 8);
...
function buildStaffCandidateActionPlan(
  detail: Awaited<ReturnType<typeof getCandidateDetail>> | null,
) {
  ...
  const actions = [
    candidate.approved === 0
      ? { title: "Review approval", subtitle: "Candidate is still waiting on a staff decision.", priority: "Critical" }
      : null,
    candidate.is_incomplete_profile
      ? { title: "Complete profile", subtitle: "Legacy profile completeness is flagged incomplete.", priority: "High" }
      : null,
    candidate.candidate_civil_need_verification
      ? { title: "Resolve civil ID", subtitle: "Civil ID verification is still marked as required.", priority: "High" }
      : null,
