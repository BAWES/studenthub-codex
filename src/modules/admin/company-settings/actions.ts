...
export async function listAdminCompanySettings(): Promise<{ items: AdminCompanySettingsItem[] }> {
  ...
  const items = rows.map((row) => {
    ...
    if (!parsed.success) {
      console.error("[admin/company-settings] listAdminCompanySettings parse error:", parsed.error.issues);
    }
  ...
  const output = { items };
  const outputParsed = adminCompanySettingsListResultSchema.safeParse(output);
  if (!outputParsed.success) {
    console.error("[admin/company-settings] listAdminCompanySettings output error:", outputParsed.error.issues);
  }
...
export async function getAdminCompanySettings(
  ...
  const parsed = adminCompanySettingsItemSchema.safeParse(item);
  if (!parsed.success) {
    console.error("[admin/company-settings] getAdminCompanySettings parse error:", parsed.error.issues);
  }
...
export async function updateAdminCompanySettings(
  ...
  try {
    ...
    if (!outputParsed.success) {
      console.error("[admin/company-settings] updateAdminCompanySettings output error:", outputParsed.error.issues);
    }
