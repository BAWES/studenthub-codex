...
export const createMajorSchema = z.object({
  major_name_en: z.string().min(1, "English name is required").max(150),
  major_name_ar: z.string().min(1, "Arabic name is required").max(150),
});

export const updateMajorSchema = z.object({
  major_uuid: z.string().min(1, "Major UUID is required"),
  major_name_en: z.string().min(1, "English name is required").max(150),
  major_name_ar: z.string().min(1, "Arabic name is required").max(150),
});
...
export type MajorDetail = z.output<typeof majorDetailSchema>;
