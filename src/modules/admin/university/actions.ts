 ...
 */
export async function listUniversity(
  params: FormData | z.input<typeof listUniversitySchema> = {},
): Promise<ListUniversityResult> {
  ...
  const parsed = listUniversitySchema.safeParse(raw);
  if (!parsed.success) {
  ...
  const outputParsed = listUniversityResultSchema.safeParse(result);
  if (!outputParsed.success) {
    logOutputError("listUniversity", outputParsed.error.issues);
  }
...
/**
 * Soft-delete a university record.
 * Throws an error if the record does not exist.
 */
